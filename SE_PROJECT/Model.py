...
class Model():
    ...
    def setLength(self, length):
        if not length:
            raise InvalidOptionException("Length should be non-zero")
        elif length < 0:
            raise InvalidOptionException("Length should not be negative")
        else:
            self.length = length
    ...
    def generatePassword(self):
        population = []
        if(self.length < len(self.options)):
            raise InvalidOptionException("Number of options selected are greater than the length specified.")
        else:
